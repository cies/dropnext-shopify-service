...
* `POST /products/fetch` — accepts `FetchProductsRequest` (at most ten product ids); answers the products, with every page of their variants, as the `ProductVariantItem`s a product webhook would upsert, the ids Shopify no longer has, and how long to wait before the next call. One Shopify failure fails the whole call, and so does running past 25 seconds.
